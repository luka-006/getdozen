...
function describe(payload: StatusPayload): {
  ...
  const deploying = /deploying/i.test(desc);
  return { rateLimited, deploying, vercel: desc };
}
...
async function main() {
  ...
  for (;;) {
    ...
    const { rateLimited, deploying, vercel } = describe(payload);
    const ts = new Date().toISOString();
    console.log(`[${ts}] ${REF} → ${payload.state}: ${vercel}`);

    if (deploying) {
      if (!wait) {
        console.log("\nDeploy in progress. Re-run with --wait.");
        process.exit(2);
      }
    ...
    if (!rateLimited && payload.state === "success") {
      console.log("\n✓ Deploy healthy. Verify: npm run audit:prod");
      return;
    }

    if (!rateLimited) {
      console.log("\n✓ Rate limit clear. Redeploy if needed:");
